...
// import { useActiveSessions, useActiveSessionsCounts } from '../hooks/useActiveSessions';
...
//   // Fetch counts for one-to-one and group sessions
//   const { oneToOneCount, groupCount, isLoading: isCountsLoading, error: countsError } = useActiveSessionsCounts();

//   // Fetch one-to-one sessions
//   const { data: oneToOneData, isLoading: isOneToOneLoading, error: oneToOneError } = useActiveSessions({
//     sortBy: 'timeIn',
//     order: sortOrder,
//     type: 'ONE_TO_ONE',
//   });

//   // Fetch group sessions (grouped)
//   const { data: groupData, isLoading: isGroupLoading, error: groupError } = useActiveSessions({
//     sortBy: 'timeIn',
//     order: sortOrder,
//     type: 'GROUP',
...
//   // Fetch admissions and activities for GroupSessionModal
...
//           apiClient.get('/api/admissions/active'),
//           apiClient.get('/api/activities/active'),
//         ]);
//         setAdmissions(admRes || []);
//         setActivities(actRes || []);
//       } catch (error) {
//         console.error('Failed to load admissions or activities:', error);
//         toast.error('Failed to load data for group sessions.');
//       }
//     };
//     loadData();
//   }, []);
...
//   const oneToOneSessions = useMemo(() => oneToOneData?.sessions || [], [oneToOneData]);
//   const groupSessions = useMemo(() => (groupData as any)?.groups || [], [groupData]);
//   const sessionsToDisplay = useMemo(() => oneToOneSessions.slice(0, 6 - groupSessions.length), [oneToOneSessions, groupSessions]);
...
//           <button className="btn btn-outline hover:bg-teal-500 hover:text-white" onClick={toggleSortOrder}>
...
//       <Modal show={showEndAllOneToOneModal} handleClose={() => setShowEndAllOneToOneModal(false)} ariaLabel="Confirm End All One-to-One Sessions">
...
//             </svg>
//           </div>
//           <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">End All One-to-One Sessions</h3>
...
//       <Modal show={showEndAllGroupModal} handleClose={() => setShowEndAllGroupModal(false)} ariaLabel="Confirm End All Group Sessions">
//         <div className="p-4 text-center">
//           <div className="mb-4">
//             <svg className="w-12 h-12 mx-auto text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
//             </svg>
//           </div>
//           <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">End All Group Sessions</h3>
//           <p className="text-gray-600 dark:text-gray-400 mb-4">
...
const ActiveSessionsDashboard: React.FC = () => {
  ...
  const oneToOneSessions = useMemo(
    () => oneToOneData?.sessions || [],
    [oneToOneData],
  );
  const groupSessions = useMemo(
    () => (groupData as any)?.groups || [],
    [groupData],
  );
  ...
  const endSessionMutation = useMutation({
    mutationFn: async (sessionId: number) => {
      const response = await apiClient.post('/api/sessions/end', {
        id: sessionId,
      });
      return response;
    },
