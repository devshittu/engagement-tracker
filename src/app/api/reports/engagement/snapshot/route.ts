...
//     endDate = period === 'month' ? endOfMonth(startDate) : startDate; // Adjust for other periods if needed
//   } catch (error) {
...
//   try {
//     log('REPORTS:ENGAGEMENT:SNAPSHOT', 'Fetching engagement snapshot', { period, startDate, endDate });
...
//     log('REPORTS:ENGAGEMENT:SNAPSHOT', 'Data prepared', { wardCount: snapshot.length, sample: snapshot.slice(0, 1) });
//     return NextResponse.json(response);
//   } catch (error) {
//     log('REPORTS:ENGAGEMENT:SNAPSHOT', 'Failed to fetch data', { error: error.message });
//     return NextResponse.json({ error: 'Failed to fetch engagement snapshot' }, { status: 500 });
//   }
// }
