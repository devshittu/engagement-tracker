...
// const log = (message: string, data?: any) => console.log(`[API:SESSIONS/ID] ${message}`, data ? JSON.stringify(data, null, 2) : '');
...
//   try {
//     const { status, timeOut, cancelReason } = await req.json();
//     log('Updating session', { id: sessionId, status, timeOut, cancelReason });
...
//       return NextResponse.json({ error: 'Session not found' }, { status: 404 });
//     }
//     log('Failed to update session', error);
...
//       return NextResponse.json({ error: 'Session not found' }, { status: 404 });
//     }
//     log('Failed to delete session', error);
