...
const log = (message: string, data?: any) =>
  console.log(`[API:AUTH/LOGIN] ${message}`, data ? JSON.stringify(data, null, 2) : '');
...
export async function POST(req: NextRequest) {
  try {
    ...
    const primaryRole = roles.length > 0 ? roles[0] : { id: 0, name: 'Unknown', level: 0 };
    ...
    log('Unexpected error during login', { error: error instanceof Error ? error.message : String(error) });
